...
#read_csv2 optimisé par rapport à read.csv
```
...
#diversité asymptotique --> diversité observé si l'inventaire avait été + important
#Sp à 1 échantillon --> diversité asymptotique = ajout du nb de sp obs 1 fois au nb d'observations totales
```
...
paracou6 %>% 
  ...
  pull(spName) %>%                    #pull transforme une colonne en vecteur
  # Création d'un objet "abundances"
  as_abundances() %>% 
  # Profil de diversité
  profile_hill() %>% 
  # Figure
  autoplot()
#Hill --> même diversité que les données, sp rares comptent bcp
```
